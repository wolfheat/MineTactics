...
public class FirestoreManager : MonoBehaviour
{
    ...
    public void Store(string val, string levelName)
    {
        ...
        LevelData levelData = new LevelData {
            Level = val,
            LevelId = levelName, // <- this is overridden in next step to unique value
            PlayCount = 0,
            Status = "Pending",
            Upvotes = 0,
            Downvotes = 0,
            DifficultyRating = 0,
            CreatorId = USerInfo.Instance.email
        };
    ...
    {
        LevelDataCollection collection = ConvertLevelsToCollection(DownloadedLevels);
